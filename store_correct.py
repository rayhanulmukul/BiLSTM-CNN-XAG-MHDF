...
def plot_timeline(ax, dates, actual, pred, future_dates, future_pred, title):
    """Plot timeline with historical and future predictions"""
    # Historical data
    ax.plot(dates, actual, label="Actual", linewidth=2.5,
            color='#2c3e50', marker='o', markersize=5, zorder=3)
    ax.plot(dates, pred, label="Predicted", linewidth=2.5,
            color='#3498db', alpha=0.85, marker='s', markersize=4, linestyle='--', zorder=2)
    
    # Future predictions
    ax.plot(future_dates, future_pred, label="Future Prediction", linewidth=2.5,
            color='#e74c3c', alpha=0.85, marker='s', markersize=4, linestyle=':', zorder=2)
...
def plot_metrics_comparison(ax, actual, pred, future_1m=None, future_3m=None, future_1y=None, model_name="Model"):
    """Plot comprehensive metrics comparison including SMAPE.

    future_1m/3m/1y are optional arrays of future predictions. If provided and
    their lengths are compatible with the end of `actual`, metrics will be
    computed against the corresponding tail of `actual`. Otherwise the value
    will be set to NaN.
    """
    metrics = {
    ...
    horizons = ['Historical', '1 Month', '3 Months', '1 Year']
    metric_values = []

    for metric_name, metric_func in metrics.items():
        # Historical (validation) metric
        try:
            historical = float(metric_func(actual, pred))
        except Exception:
            historical = np.nan

        # 1 month
        if future_1m is not None and len(future_1m) > 0 and len(actual) >= len(future_1m):
            try:
                month_1 = float(metric_func(actual[-len(future_1m):], future_1m))
            except Exception:
                month_1 = np.nan
        else:
            month_1 = np.nan
        ...
        # 3 months
        if future_3m is not None and len(future_3m) > 0 and len(actual) >= len(future_3m):
            try:
                month_3 = float(metric_func(actual[-len(future_3m):], future_3m))
            except Exception:
                month_3 = np.nan
        else:
            month_3 = np.nan
        ...
        # 1 year
        if future_1y is not None and len(future_1y) > 0 and len(actual) >= len(future_1y):
            try:
                year_1 = float(metric_func(actual[-len(future_1y):], future_1y))
            except Exception:
                year_1 = np.nan
        else:
            year_1 = np.nan

        metric_values.append([historical, month_1, month_3, year_1])

    # Plot heatmap
    sns.heatmap(metric_values, annot=True, fmt='.2f', cmap='RdYlBu_r',
                xticklabels=horizons, yticklabels=list(metrics.keys()),
                ax=ax, cbar_kws={'label': 'Metric Value'})
    ax.set_title(f'Metrics Comparison - {model_name}', fontsize=14, fontweight='bold', pad=15)
